define(['jquery'], function ($) {
  ...
  return {
    ...
    loadFonts: function (api_key) {
      $.ajax({
        url:
          'https://www.googleapis.com/webfonts/v1/webfonts?sort=style&key=' +
          api_key,
    ...
    getBreeds: function () {
      $.ajax({
        ...
        error: function (result) {
          $('.js-select-breed').html(
            '<option value="erro">Sorry, we got a problem :(</option>',
          );
        },
      });
    },
      ...
      if (local_dogs.length && typeof local_dogs !== 'undefined') {
        ...
        setTimeout(function () {
          $.each(local_dogs, function (index, dog) {
            ...
            setTimeout(function () {
              $(".list-dogs-item[data-index='" + index + "']").css(
                'opacity',
                '1',
              );
            }, 550 * index);
